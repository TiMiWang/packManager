...
public class X86EncryptHandler implements IEncryptHandler {
	...
	private String startExePath = "/host/ide/tools/eclipse/plugins/org.eclipse.cdt.core.win32_5.2.0.201102110609/os/win32/x86/starter.exe";
...
//		File spawnerFile = new File(platformPath, "LambdaIDE/eclipse/plugins/org.eclipse.cdt.core.win32.x86_5.4.0.201509131935/os/win32/x86/spawner.dat");
		File vid1DatFile = new File(platformPath,"eclipse/plugins/com.coretek.tools.ide.configManager_1.0.0/vid.dat");
		
		File settingsFile = new File(platformPath,"eclipse/configuration/.settings");
		File runtimeFile = new File(platformPath,"eclipse/configuration//org.eclipse.core.runtime");
		File appFile = new File(platformPath,"eclipse/configuration//org.eclipse.equinox.app");
		File osgiFile = new File(platformPath,"eclipse/configuration//org.eclipse.osgi");
		File updateFile = new File(platformPath,"eclipse/configuration//org.eclipse.update");
...
//			FileUtils.delFile(spawnerFile.getAbsolutePath());
	...
	public boolean dcptFileGEN(int packModeId) {
		...
		String jarPath = platformPath
				+ "/"
				+ "host/ide/LambdaPRO/eclipse/plugins/com.coretek.tools.ide.configManager_1.0.0/configManager.jar";
		...
		try {
			...
			String filePath = tempPMIdFile.getAbsolutePath() + "/"
					+ "com/coretek/tools/ide/configmanager" + "/" + "ConfigManagerPlugin.class";
			String destPath = platformPath + "/"
					+ "host/ide/platform/eclipse/jre/bin/client";
			FileUtils.copyFiletoDir(filePath, destPath);
			File newDcptFile = new File(destPath, "dcpt.jsa");
			if (newDcptFile.exists()) {
				newDcptFile.delete();
			}
			new File(destPath, "ConfigManagerPlugin.class").renameTo(newDcptFile);
		} catch (Exception e) {
			e.printStackTrace();
			status = false;
		} finally {
	...
	public boolean sapFileGEN(int packModeId) {
		...
		try {
			...
			new File(destPath, "ConfigManagerPlugin.class").renameTo(new File(destPath,
					"sap.jsa"));
		} catch (Exception e) {
			e.printStackTrace();
			status = false;
			...
			try {
				...
				if (memeroyThreadModify(arch, systemVersion, versionInfo,
					...
					if ((process.waitFor() == 0)) {
						...
						if (vidFile.exists()) {
							// 拷貝vid.dat到平臺目錄
							String newVidPath = platformPath+ "/"+ "host/ide/LambdaPRO/eclipse/plugins/com.coretek.tools.ide.configManager_1.0.0";
							FileUtils.copyFiletoDir(vidFile.getAbsolutePath(),
									newVidPath);
						} else {
							status = false;
						}
